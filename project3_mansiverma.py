...
def is_game_over(hidden_word, letters_guessed, number_of_incorrect_guesses):
    if number_of_incorrect_guesses >= 6:
        return True
    if all(letter in letters_guessed for letter in set(hidden_word)):
        return True
    return False
...
def ask_user_for_guess(letters_guessed):
    while True:
        ...
        if len(guess) == 1 and guess.isalpha() and guess not in letters_guessed:
            ...
            print("Invalid input. Please enter a single letter that hasn't been guessed.")

def print_hidden_word(hidden_word, letters_guessed):
    display_word = [letter if letter in letters_guessed else '_' for letter in hidden_word]
...
def hangman_game():
    ...
    while not is_game_over(hidden_word, letters_guessed, incorrect_guesses):
    ...
    if incorrect_guesses >= 6:
        print(f"Sorry! You're out of guesses. The word was: {hidden_word}")
    else:
